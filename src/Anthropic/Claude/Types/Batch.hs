...
instance ToJSON BatchRequest where
  toJSON (BatchRequest cid params) =
    object
      ["custom_id" .= cid, "params" .= params]

-- | Request to create a new batch.
newtype CreateBatchRequest = CreateBatchRequest
...
data BatchResponse = BatchResponse
  ...
  -- ^ URL to download results (when @Ended@)
  }
  deriving (Eq, Show, Generic)
...
instance ToJSON BatchResponse where
  toJSON (BatchResponse bid typ status counts created ended resultsUrl) =
    object
      $ [ "id" .= bid
        , "type" .= typ
...
instance ToJSON BatchResultData where
  toJSON (SuccessResult msg) =
    object
      ["type" .= ("succeeded" :: Text), "message" .= msg]
  toJSON (ErrorResult err) =
    object
      ["type" .= ("errored" :: Text), "error" .= err]
  toJSON ExpiredResult =
    object
      ["type" .= ("expired" :: Text)]
